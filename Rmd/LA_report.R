...
laz = c("New Forest", "Bath","Wiltshire","Manchester", "Leeds")
laz = "Bristol"
for (l in laz){
...
lsoa_boundaries_21 = select(IMD_2025,LSOA21CD,LSOA21NM,geom = SHAPE) |> 
  st_transform(4326)
...
cas_df_all = cas_df_msoa_all |> 
  st_set_geometry(NULL) |> 
  ungroup() |> 
  select(imd_weighted,msoa21hclnm, fatal_pcap,serious_pcap,slight_pcap,ksi_pcap,total_pcap) |> 
...
la_top8 = LA_cas_home |> 
  ...
  select(casualty_type,imd_weighted, lcs$severity) |> 
...
cc = crashes |> 
  ...
  select(collision_index,cost_per_collision) |> 
  inner_join(veh_col)
