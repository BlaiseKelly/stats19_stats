...
load(paste0("all_years_",report_casualty,".RData"))
  
  # local_area <- tmaptools::geocode_OSM("Bristol")
  # 
  # local_area_sf <- st_as_sf(st_as_sfc(local_area$bbox,crs = 4326)) |> 
  #   st_transform(27700)
  # 
  # local_area_sf <- st_read("dat/Bristol_boundary.geojson") |> 
  #   st_transform(27700)
  ...
  crashes_sf <- crashes_sf[local_area_sf,]
  
  crashes <- crashes[crashes$collision_index %in% crashes_sf$collision_index,]
  
  casualties <- casualties[casualties$collision_index %in% crashes$collision_index,]
  
  vehicles <- vehicles[vehicles$collision_index %in% crashes$collision_index,]
...
# get the extent of zones
  ...
  # pick out collisions involving report casualty
  cas_type <- casualties |> 
    filter(casualty_type == report_casualty)
  
  tot_crashes <- crashes_sf |>
    filter(collision_index %in% cas_type$collision_index) |> 
    transmute(casualties = as.numeric(number_of_casualties), geometry)
  
  
  gridded <- terra::rasterize(tot_crashes,rast(r),field = "casualties", fun = sum)
  #extent(gridded) <- r
  
  la_merc <- st_transform(local_area_sf, 3857)
  
  bg <- basemaps::basemap_raster(ext=local_area_sf, map_service = "osm", map_type = "topographic",verbose = FALSE)
  bg_m <- mask(bg,la_merc)
  
  ## plot eindhoven clock board
  tm1 <- tm_shape(bg_m) + tm_rgb(col_alpha = 1)+ tm_shape(local_area_sf) + tm_polygons(lwd = 1, fill_alpha = 0, legend.show = F) +
    tm_layout(frame = FALSE,title = paste0(ca, " Combined Authority boundary"))
  
  grd_mask <- mask(gridded, local_area_sf)
  values(grd_mask)[values(grd_mask) == 0] <- NA
  
  pal <- cols4all::c4a("gmt.seis", reverse = TRUE)
  
  bg <- basemaps::basemap_raster(ext=local_area_sf, map_service = "carto", map_type = "light", verbose = FALSE)
  bg_m <- mask(bg,la_merc)
  
  bks <- c(0,1,2,4,8,20,40,80,150,300)

#   
#   bks <- seq(0,max(values(grd_mask),na.rm = TRUE),20)
#  bks <-  scale_x_log10(
#   breaks = 10^(0:4),
#   labels = log_expr_labels(0, 4)
# )
#   bks[1] <- 1
  
  ## plot eindhoven clock board
  tm2 <- tm_shape(bg_m) + tm_rgb(col_alpha = 1)+ 
    tm_shape(local_area_sf) + 
    tm_polygons(lwd = 3, fill_alpha = 0, legend.show = F) +
    tm_shape(grd_mask) + 
    tm_raster(col.scale = tm_scale_intervals(values = pal, breaks = bks),col.legend = tm_legend(title = "casualties", position = c(0.88,0.95), frame = FALSE), col_alpha = 1) +
    tm_layout(frame = FALSE, title = paste0("Location of ", tolower(report_casualty)," casualties since 2004 (log scale)"))+tm_legend(frame = FALSE)

 tm_out <- tmap_arrange(tm1,tm2)
tmap_save(tm_out,"plots/map.png", width = 5000, height = 4000, dpi = 320)
...
fat_cas_2004 <- casualties |> filter(collision_year == "2004" & casualty_severity == "Fatal" & casualty_type == report_casualty)
#casualties this year (TY)
fat_cas_TY <- casualties |> filter(collision_year == yr2calc & casualty_severity == "Fatal" & casualty_type == report_casualty)
...
ser_cas_2004 <- casualties |> filter(collision_year == "2004" & casualty_type == report_casualty)
#casualties this year (TY)
ser_cas_TY <- casualties |> filter(collision_year == yr2calc & casualty_type == report_casualty)
...
dist_travelled_2004 <- filter(trips, Year == "2004")
dist_travelled_TY <- filter(trips, Year == yr2calc)

if(dist_travelled_TY$tot_dist_million_miles<dist_travelled_2004$tot_dist_million_miles){
  miles_id <- "decreased"
  traf <- (dist_travelled_2004$tot_dist_million_miles/dist_travelled_TY$tot_dist_million_miles)-1
} else {
  miles_id <- "increased"
  traf <- (dist_travelled_TY$tot_dist_million_miles/dist_travelled_2004$tot_dist_million_miles)-1
}
...
Between 2004 and `r yr2calc`:
 ...
 - `r tolower(report_casualty)` traffic (distance `r verb`) `r miles_id` by `r round(traf*100)`%

Averaged over the period `r yr2calc-4` to `r yr2calc`:

```{r, echo = FALSE, warning=FALSE, message=FALSE}

cas_summary_1 <- cas_L5Y |> 
...
crash_cas <- inner_join(cas_summary_1, cra_L5Y)
...
dths_per_wk_fat <- crash_cas |> 
  ...
  group_by(wk,yr) |> 
  summarise(casualties = sum(Fatal)) 
...
dths_per_wk_ser <- crash_cas %>%
  ...
  group_by(wk,yr) %>%
  summarise(casualties = sum(Serious,na.rm = TRUE)) 


```

 - an average of `r round(mean(dths_per_wk_fat$casualties),2)` `r tolower(report_casualty)`s died and `r round(mean(dths_per_wk_ser$casualties, na.rm = TRUE),2)` were seriously injured (adjusted) per week in reported road collisions
...
cas_summary <- cas_L5Y |> 
  ...
  group_by(collision_index, casualty_type) |> 
   summarise(Fatal = sum(fatal_count),
            Serious = sum(casualty_adjusted_severity_serious,na.rm = TRUE),
            Slight = sum(casualty_adjusted_severity_slight,na.rm = TRUE)) |> 
  tidyr::pivot_wider(names_from = "casualty_type", values_from = c("Fatal","Serious","Slight"))

names(cas_summary) <- c("collision_index", "Fatal", "Serious", "Slight")
...
junction_pc <- cas_summary |> 
  ...
  summarise(
    fatal_casualties   = sum(Fatal, na.rm = TRUE), # work on column index instead of name because of report casualty varying
    serious_casualties = sum(Serious, na.rm = TRUE),
    slight_casualties  = sum(Slight, na.rm = TRUE)
  ) |> 
  rowwise() |> 
...
```
 ...
 - a majority of `r tolower(report_casualty)` fatalities (`r round(not_within_20$Fatalities)`%) do not occur at or within 20m of a junction compared to `r round(not_within_20$Serious)`% of all seriously injured (adjusted) casualties
...
veh_cas_sum <- cas_summary |> 
  ...
  select(collision_index, vehicle_type, number_of_vehicles, Fatal, Serious, Slight) |> 
...
```
 ...
 - `r round(single_car$pc_fat)`% of `r report_casualty` fatalities were in collisions involving a single car
...
```
 ...
 - `r round(fatal_rural$pc)`% of `r tolower(report_casualty)` fatalities occurred on rural roads compared to `r round(rural_all$pc)`% of all `r tolower(report_casualty)` casualties
...
```
 ...
 - `r round(pc_fatal_serious_male$pc_ksi*100)`% of `r tolower(report_casualty)` killed or seriously injured (KSI) (adjusted) casualties were male
...
In `r yr2calc`, `r NROW(fatal_TY)` `r tolower(report_casualty)` were killed in `r ca` combined authority, whilst `r round(sum(serious_TY$casualty_adjusted_severity_serious,na.rm = TRUE))` were reported to be seriously injured and `r as.character(round(sum(slight_TY$casualty_adjusted_severity_slight, na.rm = TRUE)))` slightly injured.

```{r, echo = FALSE, warning=FALSE, message=FALSE}

trips <- select(trips, Year, Traffic = tot_dist_million_miles)
...
dist_trav_2004 <- filter(trips, Year == 2004)
dist_trav_TY <- filter(trips, Year == yr2calc)

if(dist_trav_2004$Traffic > dist_trav_TY$Traffic){
  dw <- "decreased"
} else {
  dw <- "increased"
}
...
Table 1 and Chart 1 show that `r tolower(report_casualty)` traffic (measured by distance `r verb`) has `r dw` between 2004 and `r yr2calc` whilst fatalities, serious and slight injuries have fallen.
...
# fatalities increased or decreased
...
Between `r yr2calc-1` and `r yr2calc`, `r tolower(report_casualty)` fatalities `r pf` by `r abs(round(diff_fatal*100))`% while `r tolower(report_casualty)` traffic (distance `r verb`) `r dw` by `r round(diff_trips*100)`%.
...
chart_1 <- rates |> melt("collision_year") |> mutate(collision_year = as.integer(collision_year))
...
table_2 <- table_1 |> 
  transmute(collision_year,
            Fatal = round(Fatal/Traffic,3),
         Serious = round(Serious/Traffic,3),
         Slight = round(Slight/Traffic,3),
         All = round(All/Traffic,3))
...
# 3. How far do `r report_casualty`s travel?
The National Travel Survey (NTS) which provides the [number of trips and average distance travelled](https://www.gov.uk/government/statistical-data-sets/nts03-modal-comparisons) (NTS0303_ca) by person per year for Combined Authority residents. This is used to derive casualty rates per mile traveled for `r tolower(report_casualty)`, which also use the landsat global 1km population data to estimate the population of the Combined Authority.

# 4. Casualty rates per mile travelled
The `r tolower(report_casualty)` casualty rate has `r pcr` for all severities in `r yr2calc` compared to 2004.
...
chart_2 <- rates_2 |> 
  melt("collision_year") |> 
  filter(!variable == "All")|> 
  mutate(collision_year = as.integer(collision_year))
...
gt(table_2,auto_align = FALSE) |> 
  cols_label(collision_year = md("**Year**"),
             ...
             All = md("**All**")) |>  
  tab_header(
    title = md(paste0("**Table 2: Casualty rates of ", tolower(report_casualty)," casualties by severity per million miles ",verb," ", ca, " CA: 2004 to ",yr2calc,"**"))) |> 
  tab_options(heading.align = "left",
              column_labels.border.top.style = "none",
              table.border.top.style = "none",
              column_labels.border.bottom.style = "none",
...
```
Between `r yr2calc-4` and `r yr2calc`, `r round(male_tot$pc*100)`% of `r tolower(report_casualty)` casualties were male and `r round(female_tot$pc*100)`% female.

There are `r round(male_times, 1)` times more male than female `r tolower(report_casualty)` casualties overall. This compares to `r round(sac_ab1$All[3]/sac_ab1$All[2],1)` more for `r ab1`,  `r round(sac_ab2$All[3]/sac_ab2$All[2],1)` more for `r ab2`and `r round(sac_ab3$All[3]/sac_ab3$All[2],1)` more for `r ab3` - the only age group in which female casualties outnumber males.
...
ggplot(sac_all, aes(x = age_band, y = pc_ksi, fill = sex_of_casualty)) + 
  ...
  ggtitle(paste0("Chart 3: Percentage of ", tolower(report_casualty), " KSI casualties, by sex and age, ", ca, " CA: 2004 to ",yr2calc,"**")) +
  dft_theme +
  theme(
...
Between `r yr2calc-4` and `r yr2calc`, most `r tolower(report_casualty)` fatalities occurred in  `r all_vehicles$number_of_vehicles[1]` vehicle collisions involving  a `r all_vehicles$summary_group[1]` (`r all_vehicles$Fatal[1]`).
...
gt(table_3,auto_align = FALSE) |> 
  ...
  tab_header(
    title = md(paste0("**Table 3: ",report_casualty," casualties in reported road collisions by severity showing other vehicles involved ",ca," CA: ", yr2calc-4, " to ",yr2calc,"**"))) |> 
  tab_options(heading.align = "left",
              column_labels.border.top.style = "none",
              table.border.top.style = "none",
              column_labels.border.bottom.style = "none",
...
crash_time %>% 
  ...
  scale_x_continuous(expand = c(0, 0)) +
  ggtitle(paste0("Chart 4: Reported ", tolower(report_casualty), " KSIs by hour of day and day of week, ",ca," CA: ", yr2calc-4, " to ", yr2calc)) +
   ylab(NULL)+
  labs(x = "Hour starting", caption = "Source: Stats19")

```

The weekday peak time for `r tolower(report_casualty)` KSIs is from `r min(MF_peak$hr[1:4])` to `r max(MF_peak$hr[1:4])`. By contrast, the peak is from `r min(SS_peak$hr[1:4])` to `r max(SS_peak$hr[1:4])` at weekends.
...
Chart 5 shows that between `r yr2calc-4` and `r yr2calc`, `r round(fatal_urban$pc)`% of `r tolower(report_casualty)` fatalities occurred on urban roads compared to `r round(all_cas$pc)`% of all `r tolower(report_casualty)` casualties. `r round(fatal_mway$pc)`% of `r tolower(report_casualty)` fatalities occurred on motorways. This would be people outside their vehicles whether they are moving at the time or not.
...
ggplot(road_type_bar, aes(fill=road_class, y=pc, x=name, label = paste0(round(pc),"%"))) +
  ...
  xlab(NULL)+
  ylab(NULL)+
  ggtitle(paste0("Chart 5: Percentage of ",tolower(report_casualty)," casualties, by urban or rural classification and severity, ",ca," CA: ", yr2calc-4, " to ", yr2calc)) +
  labs(caption = "Source: Stats19")
...
if (yr2calc > 2023) {
  cat("A majority of ", tolower(report_casualty)," fatalities(", round(highest_fat$Fatalities[1]), "%) occur ",tolower(highest_fat$Junction[1])," compared to ", round(highest_sev$Serious[1]),"% of serious injuries (adjusted). However, ", round(sum(junc$Fatalities)), "% of fatalities occur at a junction compared to ", round(sum(junc$Serious)),"% of serious injuries (adjusted).") 
} else {
  cat("A majority of ", tolower(report_casualty)," fatalities", round(highest_fat$Fatalities[1]), "%) occur ",tolower(highest_fat$Junction[1])," compared to ", round(highest_sev$Serious[1]),"% of serious injuries (adjusted). However, ", round(sum(junc$Fatalities)), "% of fatalities occur at a junction compared to ", round(sum(junc$Serious)),"% of serious injuries (adjusted). ",report_casualty," fatalities at roundabouts represent ", round(sum(roundabouts$Fatalities)), "% of all fatalities in contrast to ", round(sum(roundabouts$Serious)),"% of serious injuries (adjusted).") 
}
...
gt(table_4,auto_align = FALSE) |> 
  cols_label(Junction = md("**Junction**"),
             ...
             All = md("**All casualties**")) |>  
  tab_header(
    title = md(paste0("**Table 4: Percentage of ",tolower(report_casualty)," KSI casualties by severity and junction detail where the collision occurred, ",ca," CA: ", yr2calc-4, " to ",yr2calc,"**"))) |> 
  tab_options(heading.align = "left",
              column_labels.border.top.style = "none",
              table.border.top.style = "none",
              column_labels.border.bottom.style = "none",
